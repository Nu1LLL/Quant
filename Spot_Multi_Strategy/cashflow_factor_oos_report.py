"""Run the preregistered COWZ/VTI free-cash-flow factor first-read OOS study."""
...
def evaluate_oos(returns):
    validation = strict_validation.evaluate_strict_oos(
        returns, independent_oos=True, costs_included=True, minimum_years=9.5
    )
    folds = validation["walk_forward_folds"]
    validation["checks"]["walk_forward_passed"] = bool(
        len(folds) >= 8 and not folds.empty and folds["fold_pass"].mean() >= 0.80
    )
...
def run_experiment(prices, simulations=5000):
    ...
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 9.5:
        raise ValueError("Cash-flow factor panel must span at least 9.5 years")
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=20161216
    )
    periods = pd.Series([
        "2016_2019" if year <= 2019 else
