"""Run the preregistered BDRY freight-futures ETF first-read OOS study."""
...
LATEST_ACCEPTABLE_START = pd.Timestamp("2018-04-30", tz="UTC")


def run_experiment(prices, simulations=5000):
    prices = bdry_oos.validate_prices(prices)
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 5.0:
        raise ValueError("BDRY history must span at least five years")
    inception_covered = prices.index[0] <= LATEST_ACCEPTABLE_START
    results = {}
    for leverage in (1.0, 2.0, 3.0):
        returns, detail = bdry_oos.run_scenario(prices, leverage)
        validation = bdry_oos.evaluate_five_year_oos(returns)
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=20180322
    )
    ...
    event_label = pd.Series([
        str(year) if year in (2020, 2021, 2022, 2023, 2024, 2025, 2026)
...
def main():
    ...
    output = root / "reports/mini_medallion_bdry_oos"
    inputs = output / "inputs"
    prices = load_adjusted_close(
        "BDRY", "2017-01-01", "2026-09-12", cache_folder=inputs, refresh=False
    )
