"""Run the preregistered SCHD/VTI dividend-quality first-read OOS study."""
...
def evaluate_oos(returns):
    validation = strict_validation.evaluate_strict_oos(
        returns, independent_oos=False, costs_included=True, minimum_years=14.5
    )
...
def run_experiment(prices, simulations=5000):
    ...
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 14.5:
        raise ValueError("Dividend-quality panel must span at least 14.5 years")
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=20111020
    )
    periods = pd.Series([
        "2011_2015" if year <= 2015 else
...
def main():
    ...
    output = root / "reports/mini_medallion_dividend_quality_oos"
