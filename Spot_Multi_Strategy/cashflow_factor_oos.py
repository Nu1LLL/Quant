"""Dollar-neutral COWZ versus VTI free-cash-flow factor model."""
import numpy as np
import pandas as pd


SYMBOLS = ("COWZ", "VTI")
LATEST_ACCEPTABLE_START = pd.Timestamp("2017-01-31", tz="UTC")
...
def validate_prices(prices):
    ...
    if frame.empty or frame.index.has_duplicates or not np.isfinite(values).all():
        raise ValueError("Cash-flow panel is empty, duplicated or non-finite")
    if (values <= 0).any():
        raise ValueError("Cash-flow factor prices must be positive")
    gaps = frame.index.to_series().diff().dropna().dt.days
    if not gaps.empty and int(gaps.max()) > 10:
        raise ValueError("Cash-flow common panel has a gap longer than ten days")
    return frame
...
def run_scenario(
    ...
    target = pd.DataFrame({"COWZ": 0.5, "VTI": -0.5}, index=prices.index)
    ...
    detail = pd.DataFrame({
        "weight_COWZ": weights["COWZ"], "weight_VTI": weights["VTI"],
