"""Frozen account model for the BLNDX multi-asset NAV audit."""
import numpy as np
import pandas as pd
...
FROZEN_LEVERAGES = (1.0, 1.5, 2.0, 2.5, 3.0, 4.0)


def validate_prices(prices, maximum_gap_days=10):
    prices = pd.Series(prices).dropna().astype(float).sort_index()
    if prices.index.has_duplicates:
        raise ValueError("Duplicate BLNDX NAV date")
    if not np.isfinite(prices).all() or (prices <= 0).any():
        raise ValueError("BLNDX adjusted NAV must be finite and positive")
    gaps = prices.index.to_series().diff().dt.days.dropna()
    if len(gaps) and gaps.max() > maximum_gap_days:
        raise ValueError(f"Abnormally long BLNDX NAV gap: {int(gaps.max())} days")
    return prices
...
def evaluate_actual_path(
    ...
    walk_forward = bool(
        len(folds) >= 5 and len(folds) > 0 and folds["fold_pass"].mean() >= 0.80
    )
    ...
    checks["sample_at_least_6_25y"] = elapsed_years >= 6.25
...
def circular_block_monte_carlo_chunked(
    returns, simulations=5000, block_length=63, seed=20191230,
    batch_size=100,
):
