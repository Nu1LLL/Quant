"""Run the preregistered SRRIX reinsurance-risk-premium first-read OOS study."""
...
def run_experiment(prices, simulations=5000):
    prices = srrix_oos.validate_prices(prices)
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 10.0:
        raise ValueError("SRRIX history must span at least ten years")
    underlying_nav_returns = prices.pct_change(fill_method=None).dropna()
    results = {}
    for leverage in (1.0, 2.0, 3.0):
        returns, detail = srrix_oos.run_scenario(prices, leverage)
        results[leverage] = {
            "returns": returns,
            "detail": detail,
            "validation": srrix_oos.evaluate_ten_year_oos(
                returns, nav_returns=underlying_nav_returns
            ),
        }
    primary = results[1.0]["returns"]
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=24494897
    )
    period_label = pd.Series(
        ["through_2019" if year <= 2019 else "2020_plus" for year in primary.index.year],
        index=primary.index,
    )
    event_label = pd.Series([
        str(year) if year in (2017, 2018, 2020, 2022, 2023) else "other_days"
...
def main():
    ...
    output = root / "reports/mini_medallion_srrix_oos"
    inputs = output / "inputs"
    prices = load_adjusted_close(
        "SRRIX", "2012-01-01", "2026-09-12", cache_folder=inputs, refresh=False
    )
    ...
    for leverage, result in results.items():
        ...
        quality = result["validation"]["nav_quality"]
        rows.append({
            "leverage": leverage,
            **{f"oos__{k}": v for k, v in result["validation"]["metrics"].items()},
            **{f"nav__{k}": v for k, v in quality.items() if not isinstance(v, bool)},
    ...
    print(summary[["leverage", "oos__cagr", "oos__sharpe_ratio", "oos__max_drawdown",
                   "oos__profit_factor", "nav__zero_return_ratio",
                   "nav__lag1_autocorrelation", "check__walk_forward_passed",
