"""Run the preregistered KRBN carbon-allowance ETF first-read OOS study."""
...
LATEST_ACCEPTABLE_START = pd.Timestamp("2020-08-15", tz="UTC")


def run_experiment(prices, simulations=5000):
    prices = krbn_oos.validate_prices(prices)
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 5.0:
        raise ValueError("KRBN history must span at least five years")
    inception_covered = prices.index[0] <= LATEST_ACCEPTABLE_START
    results = {}
    for leverage in (1.0, 2.0, 3.0):
        returns, detail = krbn_oos.run_scenario(prices, leverage)
        validation = krbn_oos.evaluate_five_year_oos(returns)
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=28284271
    )
    period_label = pd.Series(
        ["inception_2022" if year <= 2022 else "2023_plus" for year in primary.index.year],
        index=primary.index,
    )
    event_label = pd.Series([
        str(year) if year in (2021, 2022, 2023, 2024, 2025, 2026) else "other_days"
        for year in primary.index.year
    ], index=primary.index)
    regimes = pd.concat([
        strict_validation.regime_metrics(primary, period_label).assign(family="period"),
        strict_validation.regime_metrics(primary, event_label).assign(family="calendar_year"),
    ], ignore_index=True)
...
def main():
    ...
    output = root / "reports/mini_medallion_krbn_oos"
    inputs = output / "inputs"
    prices = load_adjusted_close(
        "KRBN", "2020-01-01", "2026-09-12", cache_folder=inputs, refresh=False
    )
