"""Frozen account model for the QMHNX high-vol managed-futures NAV audit."""
import numpy as np
import pandas as pd
...
def validate_prices(prices, maximum_gap_days=10):
    ...
    if prices.index.has_duplicates:
        raise ValueError("Duplicate QMHNX NAV date")
    if not np.isfinite(prices).all() or (prices <= 0).any():
        raise ValueError("QMHNX adjusted NAV must be finite and positive")
    gaps = prices.index.to_series().diff().dt.days.dropna()
    if len(gaps) and gaps.max() > maximum_gap_days:
        raise ValueError(f"Abnormally long QMHNX NAV gap: {int(gaps.max())} days")
    return prices
