"""Run the preregistered RLY multi-asset real-return fund first-read
OOS study. Reuses qai_oos.validate_prices/run_scenario (same
single-fund leverage/financing model as QAI/DBV/SRRIX/MERFX/DBMF/
GLD/WTMF) and the standard 5-year strict_validation gate (RLY's
~14.4-year history doesn't clear the 15-year enhanced threshold).
"""
...
def run_experiment(prices, simulations=5000):
    ...
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 5.0:
        raise ValueError("RLY history must span at least five years")
    results = {}
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=20120426
    )
...
def main():
    ...
    output = root / "reports/mini_medallion_rly_oos"
    inputs = output / "inputs"
    prices = load_adjusted_close(
        "RLY", "2012-04-01", "2026-09-12", cache_folder=inputs, refresh=False
    )
