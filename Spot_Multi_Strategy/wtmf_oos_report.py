"""Run the preregistered WTMF managed-futures/CTA replication OOS
study. Reuses qai_oos.validate_prices/run_scenario (same single-fund
leverage/financing model as QAI/DBV/SRRIX/MERFX/DBMF) and
dbv_oos.evaluate_fifteen_year_oos (same 15-year enhanced gate as DBV,
since WTMF's ~15.7-year history barely supports it).
"""
...
def run_experiment(prices, simulations=5000):
    ...
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 15.0:
        raise ValueError("WTMF history must span at least fifteen years")
    results = {}
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=20110105
    )
...
def main():
    ...
    output = root / "reports/mini_medallion_wtmf_oos"
    inputs = output / "inputs"
    prices = load_adjusted_close(
        "WTMF", "2011-01-01", "2026-09-12", cache_folder=inputs, refresh=False
    )
