"""Run the preregistered BLNDX real multi-asset path study."""
...
def coverage_checks(prices):
    prices = blndx_actual_path.validate_prices(prices)
    ...
    return {
        "start_by_2020_03_31": prices.index[0] <= LATEST_START,
        "end_by_2026_08_31": prices.index[-1] >= EARLIEST_END,
        "span_at_least_6_25y": (
            (prices.index[-1] - prices.index[0]).days / 365.25 >= 6.25
        ),
...
def run_experiment(
    ...
    if not all(coverage.values()):
        ...
        raise ValueError(f"BLNDX coverage gate failed: {', '.join(failed)}")
    quality = blndx_actual_path.nav_quality(prices)
    results = {}
    for leverage in blndx_actual_path.FROZEN_LEVERAGES:
        returns, detail = blndx_actual_path.run_scenario(
            prices, leverage, entry_sales_load=entry_sales_load,
            exit_sales_load=exit_sales_load,
        )
        validation = blndx_actual_path.evaluate_actual_path(
    ...
    monte_carlo = blndx_actual_path.circular_block_monte_carlo_chunked(
        primary, simulations=simulations, block_length=63, seed=20191230
    )
    period = pd.Series([
        "inception_2021" if year <= 2021 else
        "2022_2023" if year <= 2023 else "2024_plus"
        for year in primary.index.year
    ], index=primary.index)
    event = pd.Series([
        str(year) if year in (2020, 2022, 2025) else "other_days"
        for year in primary.index.year
    ], index=primary.index)
    regimes = pd.concat([
...
def main():
    ...
    output = root / "reports/mini_medallion_blndx_actual_path"
    inputs = output / "inputs"
    output.mkdir(parents=True, exist_ok=True)
    prices = load_adjusted_close(
        "BLNDX", START, END, cache_folder=inputs, refresh=False
    )
