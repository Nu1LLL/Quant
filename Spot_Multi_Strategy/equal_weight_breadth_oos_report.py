"""Run the preregistered RSP/OEF equal-weight breadth first-read OOS study."""
from pathlib import Path

import pandas as pd

import equal_weight_breadth_oos as strategy
...
def evaluate_oos(returns):
    validation = strict_validation.evaluate_strict_oos(
        returns, independent_oos=True, costs_included=True, minimum_years=23.0
    )
    folds = validation["walk_forward_folds"]
    validation["checks"]["walk_forward_passed"] = bool(
        len(folds) >= 18 and not folds.empty and folds["fold_pass"].mean() >= 0.80
    )
...
def run_experiment(prices, simulations=5000):
    ...
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 23.0:
        raise ValueError("Equal-weight breadth panel must span at least twenty-three years")
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=20030424
    )
    periods = pd.Series([
        "2003_2009" if year <= 2009 else
        "2010_2017" if year <= 2017 else "2018_plus"
...
def main():
    ...
    output = root / "reports/mini_medallion_equal_weight_breadth_oos"
