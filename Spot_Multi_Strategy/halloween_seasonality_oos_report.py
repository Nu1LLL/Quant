"""Run the preregistered "Sell in May" calendar-seasonality first-read
OOS study on SPY, using DBV's 15-year enhanced gate since SPY's
...
def run_experiment(prices, simulations=5000):
    prices = hs.validate_prices(prices)
    ...
    for leverage in (1.0, 2.0, 3.0):
        net, detail = hs.run_scenario(prices, leverage)
        results[leverage] = {
            "returns": net,
            "detail": detail,
...
def main():
    ...
    output = root / "reports/mini_medallion_halloween_seasonality_oos"
