"""Five-coin basis, funding-dispersion and trend portfolio."""
...
SYMBOLS = ("DOGEUSDT", "TRXUSDT", "XLMUSDT", "ETCUSDT", "NEOUSDT")
...
def build_common_panel(frames):
    ...
    for symbol in SYMBOLS:
        ...
        pieces.append(frame[["spot", "perp_mark", "funding_rate"]].rename(columns={
            "spot": f"{symbol}__spot",
            "perp_mark": f"{symbol}__mark",
...
def coverage_audit(panel):
    ...
    audit["passed"] = bool(
        index[0] <= LATEST_ACCEPTABLE_START
        and audit["span_years"] >= 5.75
...
def _capped_inverse_vol(volatility, signs, cap=0.35):
...
def sleeve_positions(panel):
    ...
    basis_spot_target = pd.DataFrame(0.08, index=panel.index, columns=SYMBOLS)
    basis_perp_target = pd.DataFrame(-0.08, index=panel.index, columns=SYMBOLS)

    dispersion_target = pd.DataFrame(0.0, index=panel.index, columns=SYMBOLS)
    for timestamp, row in funding.iterrows():
        ordered = sorted(SYMBOLS, key=lambda symbol: (float(row[symbol]), symbol))
        dispersion_target.at[timestamp, ordered[0]] = 0.15
        dispersion_target.at[timestamp, ordered[-1]] = -0.15

    mark_returns = marks.pct_change(fill_method=None)
    momentum = marks / marks.shift(270) - 1.0
    direction = momentum.gt(0).astype(float) * 2.0 - 1.0
    direction = direction.where(momentum.notna())
    volatility = mark_returns.rolling(90, min_periods=90).std() * np.sqrt(1095.0)
    update = (
        (panel.index.weekday == 0)
        & (panel.index.hour == 0)
    )
    ...
    for timestamp in panel.index[update]:
        if direction.loc[timestamp].notna().all() and volatility.loc[timestamp].notna().all():
            trend_target.loc[timestamp] = 0.30 * _capped_inverse_vol(
                volatility.loc[timestamp], direction.loc[timestamp]
            )
    trend_target = trend_target.ffill().fillna(0.0)

    return {
        "basis_spot": basis_spot_target.shift(1),
        "basis_perp": basis_perp_target.shift(1),
        "dispersion_perp": dispersion_target.shift(1),
        "trend_perp": trend_target.shift(1),
    }, momentum, volatility
...
def run_scenario(panel, leverage, leg_cost=0.0005, annual_financing=0.04):
    ...
    sleeves, momentum, volatility = sleeve_positions(panel)
    spots = panel[[f"{symbol}__spot" for symbol in SYMBOLS]].copy()
    spots.columns = SYMBOLS
    marks = panel[[f"{symbol}__mark" for symbol in SYMBOLS]].copy()
    marks.columns = SYMBOLS
    funding = panel[[f"{symbol}__funding" for symbol in SYMBOLS]].copy()
    funding.columns = SYMBOLS
    ...
    basis_contribution = (
        scaled["basis_spot"] * spot_returns
        + scaled["basis_perp"] * mark_returns
        - scaled["basis_perp"] * funding
    ).sum(axis=1, min_count=1)
    dispersion_contribution = (
        scaled["dispersion_perp"] * mark_returns
        - scaled["dispersion_perp"] * funding
    ).sum(axis=1, min_count=1)
    trend_contribution = (
        scaled["trend_perp"] * mark_returns
        - scaled["trend_perp"] * funding
    ).sum(axis=1, min_count=1)
    gross_return = basis_contribution + dispersion_contribution + trend_contribution

    spot_position = scaled["basis_spot"]
    perp_position = scaled["basis_perp"] + scaled["dispersion_perp"] + scaled["trend_perp"]
    ...
    last = panel.index[-1]
    trading_cost.loc[last] += (
        spot_position.loc[last].abs().sum() + perp_position.loc[last].abs().sum()
    ) * leg_cost
    ...
    detail = pd.DataFrame({
        "basis_contribution": basis_contribution,
        "dispersion_contribution": dispersion_contribution,
        "trend_contribution": trend_contribution,
        "gross_return": gross_return, "spot_turnover": spot_turnover,
        "perp_turnover": perp_turnover, "trading_cost": trading_cost,
        "financing_cost": financing, "net_return": net_return,
    ...
    return daily, detail, positions.loc[detail.index], sleeves, momentum, volatility
