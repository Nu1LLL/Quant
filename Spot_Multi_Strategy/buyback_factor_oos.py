"""Dollar-neutral PKW versus IWB buyback-factor return model."""
import numpy as np
import pandas as pd
...
def validate_prices(prices):
    ...
    if frame.empty or frame.index.has_duplicates or not np.isfinite(values).all():
        raise ValueError("Buyback panel is empty, duplicated or non-finite")
    if (values <= 0).any():
        raise ValueError("Buyback-factor prices must be positive")
    gaps = frame.index.to_series().diff().dropna().dt.days
    if not gaps.empty and int(gaps.max()) > 10:
        raise ValueError("Buyback common panel has a gap longer than ten days")
    return frame
...
def run_scenario(
    ...
    target = pd.DataFrame({"PKW": 0.5, "IWB": -0.5}, index=prices.index)
    ...
    short_notional = weights["IWB"].clip(upper=0.0).abs()
    ...
    detail = pd.DataFrame({
        "weight_PKW": weights["PKW"], "weight_IWB": weights["IWB"],
