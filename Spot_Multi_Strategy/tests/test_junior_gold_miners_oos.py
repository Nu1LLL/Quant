...
class ValidatePairTests(unittest.TestCase):
    ...
    def test_rejects_duplicate_dates(self):
        ...
        with self.assertRaises(ValueError):
            jgm.validate_pair(miners, bullion)
    ...
    def test_rejects_abnormally_long_gap(self):
        ...
        with self.assertRaises(ValueError):
            jgm.validate_pair(series, series)

    def test_keeps_only_overlapping_dates(self):
        dates = self._dates(5)
        miners = pd.Series([1.0] * 5, index=dates)
        bullion = pd.Series([1.0] * 3, index=dates[:3])
        aligned = jgm.validate_pair(miners, bullion)
        self.assertEqual(len(aligned), 3)
        self.assertListEqual(list(aligned.columns), ["GDXJ", "GLD"])
...
class RunScenarioTests(unittest.TestCase):
    def setUp(self):
        ...
        self.miners_returns = rng.normal(0.0005, 0.026, len(self.dates))
    ...
    def test_turnover_only_on_rebalance_days(self):
        _, detail = jgm.run_scenario(self.miners, self.bullion, leverage=1.0)
    ...
    def test_weights_reset_to_target_at_rebalance(self):
        ...
        expected_weight = 0.5 * (1 + row["junior_miners_return"]) / (
            1 + row["net_return"]
        )
    ...
    def test_identical_legs_cancel_before_cost(self):
        ...
        shared_returns = rng.normal(0.0003, 0.014, len(dates))
        miners, bullion = _prices(dates, shared_returns, shared_returns)
        net, detail = jgm.run_scenario(
    ...
    def test_leverage_scales_financing_cost(self):
        _, detail_1x = jgm.run_scenario(self.miners, self.bullion, leverage=1.0)
        _, detail_2x = jgm.run_scenario(self.miners, self.bullion, leverage=2.0)
    ...
    def test_raises_when_series_are_empty_after_alignment(self):
        ...
        with self.assertRaises(ValueError):
            jgm.run_scenario(empty, self.bullion, leverage=1.0)
