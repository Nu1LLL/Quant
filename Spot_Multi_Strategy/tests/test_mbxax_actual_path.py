...
import mbxax_actual_path
import mbxax_actual_path_report
import strict_validation


class MbxaxActualPathTests(unittest.TestCase):
    def setUp(self):
        self.index = pd.date_range("2016-01-04", periods=2800, freq="B", tz="UTC")
        rng = np.random.default_rng(1228)
        self.prices = pd.Series(
            10 * np.cumprod(1 + 0.0003 + rng.normal(0, 0.006, len(self.index))),
            index=self.index,
        )

    def test_sales_load_and_financing_are_explicit(self):
        _, one = mbxax_actual_path.run_scenario(
            self.prices, 1.0, entry_sales_load=0.0575
        )
        _, two = mbxax_actual_path.run_scenario(
    ...
    def test_future_mutation_does_not_change_past(self):
        baseline, _ = mbxax_actual_path.run_scenario(self.prices, 1.0)
        changed = self.prices.copy()
        changed.iloc[-1] *= 0.8
        rerun, _ = mbxax_actual_path.run_scenario(changed, 1.0)
    ...
    def test_nav_quality_rejects_stale_series(self):
        ...
        quality = mbxax_actual_path.nav_quality(stale)
    ...
    def test_chunked_monte_carlo_matches_shared_small_case(self):
        returns, _ = mbxax_actual_path.run_scenario(self.prices.iloc[:600], 1.0)
        expected = strict_validation.circular_block_monte_carlo(
            returns, simulations=20, block_length=21, seed=77
        )
        actual = mbxax_actual_path.circular_block_monte_carlo_chunked(
    ...
    def test_report_has_frozen_leverage_grid(self):
        results, _, _, _, _ = mbxax_actual_path_report.run_experiment(
            self.prices, simulations=20, account_executable=True,
            strategy_continuity=True, entry_sales_load=0.0575,
        )
        self.assertEqual(set(results), set(mbxax_actual_path.FROZEN_LEVERAGES))

    def test_coverage_gate_rejects_short_history(self):
        short = self.prices.loc[self.prices.index >= "2018-01-01"]
        with self.assertRaises(ValueError):
            mbxax_actual_path_report.run_experiment(short, simulations=20)
