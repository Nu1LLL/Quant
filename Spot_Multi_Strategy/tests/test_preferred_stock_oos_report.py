...
import preferred_stock_oos_report as report


class RunExperimentTests(unittest.TestCase):
    def _sixteen_year_prices(self, seed=137, drift=0.0002, scale=0.005):
        dates = pd.bdate_range("2007-03-30", periods=252 * 16, tz="UTC")
        rng = np.random.default_rng(seed)
        returns = rng.normal(drift, scale, len(dates))
        prices = 35.0 * np.cumprod(1.0 + returns)
        return pd.Series(prices, index=dates)

    def test_rejects_history_shorter_than_fifteen_years(self):
        dates = pd.bdate_range("2007-03-30", periods=252 * 10, tz="UTC")
        rng = np.random.default_rng(7)
        prices = pd.Series(
            35.0 * np.cumprod(1.0 + rng.normal(0.0002, 0.005, len(dates))),
            index=dates
        )
    ...
    def test_leverage_scales_reported_cagr_direction(self):
        prices = self._sixteen_year_prices(seed=19, drift=0.0006, scale=0.004)
