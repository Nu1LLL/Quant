...
import gold_oos_report as report


class RunExperimentTests(unittest.TestCase):
    def _sixteen_year_prices(self, seed=43, drift=0.0002, scale=0.008):
        dates = pd.bdate_range("2004-11-18", periods=252 * 16, tz="UTC")
        rng = np.random.default_rng(seed)
        returns = rng.normal(drift, scale, len(dates))
        prices = 40.0 * np.cumprod(1.0 + returns)
    ...
    def test_rejects_history_shorter_than_fifteen_years(self):
        ...
        prices = pd.Series(
            40.0 * np.cumprod(1.0 + rng.normal(0.0002, 0.008, len(dates))),
            index=dates
        )
    ...
    def test_leverage_scales_reported_cagr_direction(self):
        prices = self._sixteen_year_prices(seed=11, drift=0.0006, scale=0.005)
