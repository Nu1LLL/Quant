...
import abyax_actual_path_report
import strict_validation


class AbyaxActualPathTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_sales_load_and_financing_are_explicit(self):
        _, one = abyax_actual_path.run_scenario(
            self.prices, 1.0, entry_sales_load=0.0575
        )
        _, two = abyax_actual_path.run_scenario(
    ...
    def test_future_mutation_does_not_change_past(self):
        baseline, _ = abyax_actual_path.run_scenario(self.prices, 1.0)
        changed = self.prices.copy()
        changed.iloc[-1] *= 0.8
        rerun, _ = abyax_actual_path.run_scenario(changed, 1.0)
    ...
    def test_nav_quality_rejects_stale_series(self):
        ...
        quality = abyax_actual_path.nav_quality(stale)
    ...
    def test_chunked_monte_carlo_matches_shared_small_case(self):
        returns, _ = abyax_actual_path.run_scenario(self.prices.iloc[:600], 1.0)
        expected = strict_validation.circular_block_monte_carlo(
            returns, simulations=20, block_length=21, seed=77
        )
        actual = abyax_actual_path.circular_block_monte_carlo_chunked(
