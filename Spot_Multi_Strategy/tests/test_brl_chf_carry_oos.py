...
import brl_chf_carry_oos_report as report


class BrlChfCarryOosTests(unittest.TestCase):
    def setUp(self):
        self.index = pd.date_range("2008-05-09", periods=4700, freq="B", tz="UTC")
        rng = np.random.default_rng(509)
        common = rng.normal(0.00005, 0.005, len(self.index))
        self.prices = pd.DataFrame({
            "BZF": 25 * np.cumprod(1 + common + rng.normal(0.0001, 0.006, len(self.index))),
            "FXF": 100 * np.cumprod(1 + common + rng.normal(0.0, 0.002, len(self.index))),
        }, index=self.index)
    ...
    def test_late_start_and_short_span_fail(self):
        ...
        shifted.index = pd.date_range("2009-02-02", periods=len(shifted), freq="B", tz="UTC")
    ...
    def test_bad_panel_and_gap_are_rejected(self):
        with self.assertRaises(ValueError):
            strategy.validate_prices(self.prices[["BZF"]])
        with self.assertRaises(ValueError):
