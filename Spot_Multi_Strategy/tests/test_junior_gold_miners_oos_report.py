...
import junior_gold_miners_oos_report as report


class RunExperimentTests(unittest.TestCase):
    def _sixteen_year_prices(self, seed=191, drift_a=0.0003, drift_b=0.0002):
        dates = pd.bdate_range("2009-11-11", periods=252 * 16, tz="UTC")
        rng = np.random.default_rng(seed)
        miners = 35.0 * np.cumprod(1.0 + rng.normal(drift_a, 0.026, len(dates)))
        bullion = 35.0 * np.cumprod(1.0 + rng.normal(drift_b, 0.009, len(dates)))
    ...
    def test_rejects_history_shorter_than_fifteen_years(self):
        dates = pd.bdate_range("2009-11-11", periods=252 * 10, tz="UTC")
        rng = np.random.default_rng(9)
        miners = pd.Series(
            35.0 * np.cumprod(1.0 + rng.normal(0.0003, 0.026, len(dates))), index=dates
        )
        bullion = pd.Series(
            35.0 * np.cumprod(1.0 + rng.normal(0.0002, 0.009, len(dates))), index=dates
        )
    ...
    def test_leverage_scales_reported_cagr_direction(self):
        miners, bullion = self._sixteen_year_prices(
            seed=29, drift_a=0.0009, drift_b=0.0001
        )
