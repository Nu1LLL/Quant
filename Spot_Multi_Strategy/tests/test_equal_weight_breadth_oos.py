...
import equal_weight_breadth_oos_report as report


class EqualWeightBreadthOosTests(unittest.TestCase):
    def setUp(self):
        self.index = pd.date_range("2003-04-24", periods=6100, freq="B", tz="UTC")
        rng = np.random.default_rng(424)
        market = rng.normal(0.0003, 0.012, len(self.index))
        self.prices = pd.DataFrame({
            "RSP": 40 * np.cumprod(1 + market + rng.normal(0.0001, 0.003, len(self.index))),
            "OEF": 50 * np.cumprod(1 + market),
        }, index=self.index)
    ...
    def test_late_start_and_short_span_fail(self):
        ...
        shifted.index = pd.date_range("2003-06-02", periods=len(shifted), freq="B", tz="UTC")
    ...
    def test_bad_panel_and_gap_are_rejected(self):
        with self.assertRaises(ValueError):
            strategy.validate_prices(self.prices[["RSP"]])
        with self.assertRaises(ValueError):
