"""Frozen account model for the ARBFX merger-arbitrage NAV audit."""
import numpy as np
import pandas as pd
...
def validate_prices(prices, maximum_gap_days=10):
    ...
    if prices.index.has_duplicates:
        raise ValueError("Duplicate ARBFX NAV date")
    if not np.isfinite(prices).all() or (prices <= 0).any():
        raise ValueError("ARBFX adjusted NAV must be finite and positive")
    gaps = prices.index.to_series().diff().dt.days.dropna()
    if len(gaps) and gaps.max() > maximum_gap_days:
        raise ValueError(f"Abnormally long ARBFX NAV gap: {int(gaps.max())} days")
    return prices
...
def run_scenario(
    ...
    return cvsix_actual_path.run_scenario(
...
def evaluate_actual_path(
    ...
    walk_forward = bool(
        len(folds) >= 12 and len(folds) > 0 and folds["fold_pass"].mean() >= 0.80
    )
...
def circular_block_monte_carlo_chunked(
    returns, simulations=5000, block_length=63, seed=20000917,
    batch_size=100,
):
