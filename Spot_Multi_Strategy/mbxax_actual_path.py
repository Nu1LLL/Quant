"""Frozen account model for the MBXAX multi-market NAV audit."""
import numpy as np
import pandas as pd
...
def validate_prices(prices, maximum_gap_days=10):
    ...
    if prices.index.has_duplicates:
        raise ValueError("Duplicate MBXAX NAV date")
    if not np.isfinite(prices).all() or (prices <= 0).any():
        raise ValueError("MBXAX adjusted NAV must be finite and positive")
    gaps = prices.index.to_series().diff().dt.days.dropna()
    if len(gaps) and gaps.max() > maximum_gap_days:
        raise ValueError(f"Abnormally long MBXAX NAV gap: {int(gaps.max())} days")
    return prices
...
def circular_block_monte_carlo_chunked(
    returns, simulations=5000, block_length=63, seed=20151228,
    batch_size=100,
):
