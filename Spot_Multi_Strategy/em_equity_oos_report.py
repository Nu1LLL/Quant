"""Run the preregistered independent EM equity risk-premium OOS study
(EEM long / SPY short), using DBV's 15-year enhanced gate since
EEM/SPY's ~23-year common history supports it."""
...
def run_experiment(emerging, developed, simulations=5000):
    if (emerging.index[-1] - emerging.index[0]).days / 365.25 < 15.0:
        raise ValueError("EEM/SPY history must span at least fifteen years")
    results = {}
    for leverage in (1.0, 2.0, 3.0):
        net, detail = ee.run_scenario(emerging, developed, leverage)
        results[leverage] = {
            "returns": net,
            "detail": detail,
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=20030414
    )

    period_label = pd.Series([
        "inception_2007" if year <= 2007 else
        "2008_2015" if year <= 2015 else "2016_plus"
        for year in primary.index.year
    ], index=primary.index)
    event_label = pd.Series([
        str(year) if year in (2008, 2015, 2022) else "other_days"
        for year in primary.index.year
    ], index=primary.index)
    regimes = pd.concat([
...
def main():
    ...
    output = root / "reports/mini_medallion_em_equity_oos"
    inputs = output / "inputs"

    emerging = load_adjusted_close(
        "EEM", "2003-04-01", "2026-09-12", cache_folder=inputs, refresh=False
    )
    developed = load_adjusted_close(
        "SPY", "2003-04-01", "2026-09-12", cache_folder=inputs, refresh=False
    )

    results, monte_carlo, regimes = run_experiment(emerging, developed)

    emerging.rename_axis("date").to_csv(output / "eem_adjusted_close.csv")
    developed.rename_axis("date").to_csv(output / "spy_adjusted_close.csv")
    ...
    print("EEM rows", len(emerging), emerging.index.min(), emerging.index.max())
    print("SPY rows", len(developed), developed.index.min(), developed.index.max())
    print(summary[[
