"""Run the preregistered Dollar Index futures trend first-read OOS study."""
...
def evaluate_oos(returns):
    ...
    validation["checks"]["walk_forward_passed"] = bool(
        len(folds) >= 15 and not folds.empty and folds["fold_pass"].mean() >= 0.80
    )
...
def run_experiment(prices, simulations=5000):
    ...
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 20.0:
        raise ValueError("Dollar Index futures series must span at least twenty years")
    inception_covered = prices.index[0] <= strategy.LATEST_ACCEPTABLE_START
    results = {}
    for leverage in (1.0, 2.0, 3.0):
        returns, detail, positions, targets = strategy.run_scenario(prices, leverage)
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=20000103
    )
    periods = pd.Series([
        "2000_2008" if year <= 2008 else
        "2009_2016" if year <= 2016 else "2017_plus"
...
def main():
    ...
    output = root / "reports/mini_medallion_dxy_trend_oos"
    ...
    diagnostics = pd.DataFrame([{
        "long_days": int(primary["position"].gt(0).sum()),
        "short_days": int(primary["position"].lt(0).sum()),
        "gross_return_sum": primary["gross_return"].sum(),
        "trading_cost_sum": primary["trading_cost"].sum(),
        "roll_haircut_sum": primary["roll_haircut"].sum(),
    }])
