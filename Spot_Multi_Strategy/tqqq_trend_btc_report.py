"""Run the pre-registered tactical TQQQ and BTC study."""
...
def run_experiment(qqq, tqqq, btc, repetitions=4999):
    position = tqqq_trend_btc.build_tactical_position(qqq)
    buy_hold_position = pd.Series(1.0, index=tqqq.index)
    buy_hold = tqqq_trend_btc.run_etf_sleeve(tqqq, buy_hold_position)
    tactical = tqqq_trend_btc.run_etf_sleeve(tqqq, position)

    rows = [
        {"scenario": "TQQQ_BUY_HOLD", **buy_hold[2]},
        {"scenario": "TACTICAL_TQQQ", **tactical[2]},
    ]
    artifacts = {
        "TQQQ_BUY_HOLD": (buy_hold[0], None, buy_hold[1]),
        "TACTICAL_TQQQ": (tactical[0], None, tactical[1]),
    }
    tactical_return = tactical[0].set_index("open_time")["net_pnl"]
    simulation, yearly, metrics = blend_risk_overlay.run_overlay(
        tactical_return
    )
    rows.append({"scenario": "TACTICAL_RISK_OVERLAY", **metrics})
    artifacts["TACTICAL_RISK_OVERLAY"] = (simulation, None, yearly)

    component_returns = tqqq_trend_btc.align_with_btc(tactical[0], btc)
    ...
    summary["bootstrap_p"] = [
        circular_block_bootstrap_mean_pvalue(
            ...
            repetitions=repetitions, seed=390389
        )
        for name in summary["scenario"]
    ]
    return add_gate(summary), artifacts, component_returns, position


def main():
    root = Path(__file__).resolve().parent
    output = root / "reports/mini_medallion_tqqq_trend_btc"
    ...
    summary, artifacts, component_returns, position = run_experiment(
        prices["QQQ"], prices["TQQQ"], btc
    )
    pd.concat(prices, axis=1).rename_axis("open_time").to_csv(
        output / "adjusted_levels.csv"
    )
    position.rename("tqqq_position").rename_axis("open_time").to_csv(
        output / "tactical_position.csv"
    )
