"""Run the preregistered DBV G10 currency-carry ETF first-read OOS study."""
...
LATEST_ACCEPTABLE_INCEPTION_DATE = pd.Timestamp("2006-09-30", tz="UTC")


def run_experiment(prices, simulations=5000):
    prices = dbv_oos.validate_prices(prices)
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 15.0:
        raise ValueError("DBV history must span at least fifteen years")
    results = {}
    for leverage in (1.0, 2.0, 3.0):
        returns, detail = dbv_oos.run_scenario(prices, leverage)
        results[leverage] = {
            "returns": returns,
            "detail": detail,
            "validation": dbv_oos.evaluate_fifteen_year_oos(returns),
        }
        complete_lifetime_data = prices.index[0] <= LATEST_ACCEPTABLE_INCEPTION_DATE
        results[leverage]["validation"]["checks"][
            "complete_lifetime_data"
        ] = complete_lifetime_data
        results[leverage]["validation"]["passed"] = all(
            results[leverage]["validation"]["checks"].values()
        )
    primary = results[1.0]["returns"]
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=17320508
    )
    period_label = pd.Series([
        "inception_2012" if year <= 2012 else
...
def main():
    ...
    output = root / "reports/mini_medallion_dbv_oos"
    inputs = output / "inputs"
    prices = load_adjusted_close(
        "DBV", "2005-01-01", "2026-09-12", cache_folder=inputs, refresh=False
    )
