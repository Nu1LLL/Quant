"""Run the preregistered MBXAX multi-market path study."""
...
def coverage_checks(prices):
    prices = mbxax_actual_path.validate_prices(prices)
...
def run_experiment(
    ...
    if not all(coverage.values()):
        ...
        raise ValueError(f"MBXAX coverage gate failed: {', '.join(failed)}")
    quality = mbxax_actual_path.nav_quality(prices)
    results = {}
    for leverage in mbxax_actual_path.FROZEN_LEVERAGES:
        returns, detail = mbxax_actual_path.run_scenario(
            prices, leverage, entry_sales_load=entry_sales_load,
            exit_sales_load=exit_sales_load,
        )
        validation = mbxax_actual_path.evaluate_actual_path(
    ...
    monte_carlo = mbxax_actual_path.circular_block_monte_carlo_chunked(
        primary, simulations=simulations, block_length=63, seed=20151228
    )
    period = pd.Series([
...
def main():
    ...
    output = root / "reports/mini_medallion_mbxax_actual_path"
    inputs = output / "inputs"
    output.mkdir(parents=True, exist_ok=True)
    prices = load_adjusted_close(
        "MBXAX", START, END, cache_folder=inputs, refresh=False
    )
