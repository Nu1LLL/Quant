"""Run the preregistered QAI multi-strategy ETF first-read OOS study."""
...
def run_experiment(prices, simulations=5000):
    prices = qai_oos.validate_prices(prices)
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 10.0:
        raise ValueError("QAI history must span at least ten years")
    results = {}
    for leverage in (1.0, 2.0, 3.0):
        returns, detail = qai_oos.run_scenario(prices, leverage)
        results[leverage] = {
            "returns": returns,
            "detail": detail,
            "validation": qai_oos.evaluate_ten_year_oos(returns),
        }
    primary = results[1.0]["returns"]
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=14142135
    )
    period_label = pd.Series(
        ["inception_2019" if year <= 2019 else "2020_plus" for year in primary.index.year],
        index=primary.index,
    )
    event_label = pd.Series(
        [str(year) if year in (2020, 2022) else "other_days" for year in primary.index.year],
        index=primary.index,
    )
    regimes = pd.concat([
...
def main():
    ...
    output = root / "reports/mini_medallion_qai_oos"
    inputs = output / "inputs"
    prices = load_adjusted_close(
        "QAI", "2009-01-01", "2026-09-12", cache_folder=inputs, refresh=True
    )
