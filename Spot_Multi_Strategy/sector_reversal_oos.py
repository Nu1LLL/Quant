"""Causal weekly cross-sectional reversal across nine US sector ETFs."""
import numpy as np
import pandas as pd


SYMBOLS = ("XLB", "XLE", "XLF", "XLI", "XLK", "XLP", "XLU", "XLV", "XLY")
LATEST_ACCEPTABLE_START = pd.Timestamp("1999-01-31", tz="UTC")
...
def validate_prices(prices):
    ...
    if frame.empty or frame.index.has_duplicates or not np.isfinite(values).all():
        raise ValueError("Sector panel is empty, duplicated or non-finite")
    if (values <= 0).any():
    ...
    if not gaps.empty and int(gaps.max()) > 10:
        raise ValueError("Sector common panel has a gap longer than ten days")
    return frame


def causal_weights(prices, lookback=5):
    ...
    for timestamp in prices.index[prices.index.weekday == 2]:
        row = trailing_return.loc[timestamp]
        if row.notna().all():
    ...
    executed = targets.shift(2).fillna(0.0)
    return executed, trailing_return, targets


def run_scenario(
    prices, leverage, leg_cost=0.0002, annual_borrow=0.005,
    annual_financing=0.04,
):
    prices = validate_prices(prices)
    base_weights, trailing_return, targets = causal_weights(prices)
    ...
    return detail["net_return"], detail, weights.loc[detail.index], trailing_return, targets
