"""Run the preregistered TQQQ/SQQQ switch and frozen BTC blend."""
...
def run_experiment(qqq, tqqq, sqqq, btc, repetitions=4999):
    sleeve = tqqq_sqqq_btc.run_switch_sleeve(qqq, tqqq, sqqq)
    rows = [{"scenario": "TQQQ_SQQQ_SWITCH", **sleeve[3]}]
    artifacts = {"TQQQ_SQQQ_SWITCH": (sleeve[0], sleeve[1], sleeve[2])}
    ...
    component_returns = tqqq_sqqq_btc.align_with_btc(sleeve[0], btc)
    ...
    base_return, positions, _ = pput_btc_blend.run_base_blend(component_returns)
    ...
    summary["bootstrap_p"] = [
        circular_block_bootstrap_mean_pvalue(
            ...
            repetitions=repetitions, seed=832040
        )
...
def main():
    ...
    output = root / "reports/mini_medallion_tqqq_sqqq_btc"
    output.mkdir(parents=True, exist_ok=True)
    prices = {}
    for symbol in ("QQQ", "TQQQ", "SQQQ"):
        series = load_adjusted_close(symbol, "2015-09-01", "2026-09-11", output)
    ...
    summary, artifacts, components = run_experiment(
        prices["QQQ"], prices["TQQQ"], prices["SQQQ"], btc
    )
    pd.concat(prices, axis=1).rename_axis("open_time").to_csv(
        output / "adjusted_levels.csv"
    )
    components.rename_axis("open_time").to_csv(output / "blend_component_returns.csv")
