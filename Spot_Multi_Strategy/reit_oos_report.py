"""Run the preregistered VNQ US REIT first-read OOS study. Reuses
qai_oos.validate_prices/run_scenario (same single-fund
leverage/financing model as QAI/DBV/SRRIX/MERFX/DBMF/GLD/WTMF/RLY/
CWB) and dbv_oos.evaluate_fifteen_year_oos (same 15-year enhanced
gate as DBV, since VNQ's ~22-year history supports it).
"""
...
def run_experiment(prices, simulations=5000):
    ...
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 15.0:
        raise ValueError("VNQ history must span at least fifteen years")
    results = {}
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=20040929
    )
...
def main():
    ...
    output = root / "reports/mini_medallion_reit_oos"
    inputs = output / "inputs"
    prices = load_adjusted_close(
        "VNQ", "2004-09-01", "2026-09-12", cache_folder=inputs, refresh=False
    )
