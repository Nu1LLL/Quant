"""Run the preregistered PFIX long-rate-convexity ETF first-read OOS study."""
...
LATEST_ACCEPTABLE_START = pd.Timestamp("2021-06-30", tz="UTC")


def run_experiment(prices, simulations=5000):
    prices = pfix_oos.validate_prices(prices)
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 5.0:
        raise ValueError("PFIX history must span at least five years")
    inception_covered = prices.index[0] <= LATEST_ACCEPTABLE_START
    results = {}
    for leverage in (1.0, 2.0, 3.0):
        returns, detail = pfix_oos.run_scenario(prices, leverage)
        validation = pfix_oos.evaluate_five_year_oos(returns)
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=20210510
    )
    period_label = pd.Series([
        "2021" if year == 2021 else
        "2022" if year == 2022 else
        "2023_2024" if year <= 2024 else "2025_plus"
        for year in primary.index.year
    ], index=primary.index)
    event_label = pd.Series([
        str(year) if year in (2022, 2023, 2024, 2025, 2026)
...
def main():
    ...
    output = root / "reports/mini_medallion_pfix_oos"
    inputs = output / "inputs"
    prices = load_adjusted_close(
        "PFIX", "2021-01-01", "2026-09-12", cache_folder=inputs, refresh=False
    )
