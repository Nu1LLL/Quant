"""Frozen account model for the QGMNX global-macro NAV audit."""
import numpy as np
import pandas as pd
...
def validate_prices(prices, maximum_gap_days=10):
    ...
    if prices.index.has_duplicates:
        raise ValueError("Duplicate QGMNX NAV date")
    if not np.isfinite(prices).all() or (prices <= 0).any():
        raise ValueError("QGMNX adjusted NAV must be finite and positive")
    gaps = prices.index.to_series().diff().dt.days.dropna()
    if len(gaps) and gaps.max() > maximum_gap_days:
        raise ValueError(f"Abnormally long QGMNX NAV gap: {int(gaps.max())} days")
    return prices
...
def evaluate_actual_path(returns, solvent, account_executable):
    ...
    checks["sample_at_least_10_5y"] = elapsed_years >= 10.5
