"""Run the preregistered PKW/IWB buyback-factor first-read OOS study."""
...
def evaluate_long_oos(returns):
    validation = strict_validation.evaluate_strict_oos(
        returns, independent_oos=True, costs_included=True, minimum_years=19.5
    )
...
def run_experiment(prices, simulations=5000):
    ...
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 19.5:
        raise ValueError("Buyback common panel must span at least 19.5 years")
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=20061220
    )
    periods = pd.Series([
        "2006_2012" if year <= 2012 else
        "2013_2019" if year <= 2019 else "2020_plus"
        for year in primary.index.year
    ], index=primary.index)
    events = pd.Series([
        str(year) if year in (2008, 2018, 2020, 2022) else "other_days"
        for year in primary.index.year
    ], index=primary.index)
    regimes = pd.concat([
...
def main():
    ...
    output = root / "reports/mini_medallion_buyback_factor_oos"
