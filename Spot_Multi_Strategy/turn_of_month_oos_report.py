"""Run the preregistered turn-of-the-month first-read OOS study on
SPY, using DBV's 15-year enhanced gate since SPY's ~33.6-year history
...
def run_experiment(prices, simulations=5000):
    prices = tm.validate_prices(prices)
    ...
    for leverage in (1.0, 2.0, 3.0):
        net, detail = tm.run_scenario(prices, leverage)
        results[leverage] = {
            "returns": net,
            "detail": detail,
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=19870101
    )
...
def main():
    ...
    output = root / "reports/mini_medallion_turn_of_month_oos"
