"""Run the pre-registered TQQQ/FMF regime switch and BTC blend."""
...
def run_experiment(qqq, tqqq, fmf, btc, repetitions=4999):
    sleeve = tqqq_fmf_btc.run_switch_sleeve(qqq, tqqq, fmf)
    rows = [{"scenario": "TQQQ_FMF_SWITCH", **sleeve[3]}]
    artifacts = {"TQQQ_FMF_SWITCH": (sleeve[0], sleeve[1], sleeve[2])}
    ...
    component_returns = tqqq_trend_btc.align_with_btc(sleeve[0], btc).rename(
        columns={"TACTICAL_TQQQ": "TQQQ_FMF_SWITCH"}
    )
    ...
    summary["bootstrap_p"] = [
        circular_block_bootstrap_mean_pvalue(
            ...
            repetitions=repetitions, seed=470077
...
def main():
    ...
    output = root / "reports/mini_medallion_tqqq_fmf_btc"
    output.mkdir(parents=True, exist_ok=True)
    prices = {}
    for symbol in ("QQQ", "TQQQ", "FMF"):
    ...
    summary, artifacts, component_returns = run_experiment(
        prices["QQQ"], prices["TQQQ"], prices["FMF"], btc
    )
    pd.concat(prices, axis=1).rename_axis("open_time").to_csv(output / "adjusted_levels.csv")
    component_returns.rename_axis("open_time").to_csv(output / "blend_component_returns.csv")
    ...
    print(artifacts["TQQQ_FMF_SWITCH"][1].mean())
    columns = [
