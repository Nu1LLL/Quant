"""Run the preregistered CWB convertible-bond first-read OOS study.
...
and dbv_oos.evaluate_fifteen_year_oos (same 15-year enhanced gate as
DBV, since CWB's ~17.4-year history supports it).
"""
...
def run_experiment(prices, simulations=5000):
    ...
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 15.0:
        raise ValueError("CWB history must span at least fifteen years")
    results = {}
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=20090416
    )
...
def main():
    ...
    output = root / "reports/mini_medallion_convertible_bond_oos"
    inputs = output / "inputs"
    prices = load_adjusted_close(
        "CWB", "2009-04-01", "2026-09-12", cache_folder=inputs, refresh=False
    )
