"""Run the preregistered independent size-factor OOS study
(IWM long / SPY short), using DBV's 15-year enhanced gate since
IWM's 26-year history supports it."""
...
def run_experiment(small_cap, market, simulations=5000):
    if (small_cap.index[-1] - small_cap.index[0]).days / 365.25 < 15.0:
        raise ValueError("IWM history must span at least fifteen years")
    results = {}
    for leverage in (1.0, 2.0, 3.0):
        net, detail = sf.run_scenario(small_cap, market, leverage)
        results[leverage] = {
            "returns": net,
            "detail": detail,
...
def main():
    ...
    output = root / "reports/mini_medallion_size_factor_oos"
    ...
    results, monte_carlo, regimes = run_experiment(small_cap, market)
