"""Run the preregistered QGMNX global-macro path study."""
...
def run_experiment(prices, simulations=5000, account_executable=False):
    ...
    if not all(coverage.values()):
        ...
        raise ValueError(f"QGMNX coverage gate failed: {', '.join(failed)}")
    results = {}
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=63, seed=20150901
    )
    period = pd.Series([
...
def main():
    ...
    output = root / "reports/mini_medallion_qgmnx_actual_path"
