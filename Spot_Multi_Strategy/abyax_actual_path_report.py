"""Run the preregistered ABYAX multi-manager CTA path study."""
from pathlib import Path

import pandas as pd

import abyax_actual_path
...
START = "2014-01-01"
END = "2026-09-16"
LATEST_START = pd.Timestamp("2016-12-31", tz="UTC")
...
ENTRY_SALES_LOAD = 0.0575
EXIT_SALES_LOAD = 0.0


def coverage_checks(prices):
    prices = abyax_actual_path.validate_prices(prices)
    ...
    return {
        "start_by_2016_12_31": prices.index[0] <= LATEST_START,
        "end_by_2026_08_31": prices.index[-1] >= EARLIEST_END,
        "span_at_least_9_5y": (
            (prices.index[-1] - prices.index[0]).days / 365.25 >= 9.5
        ),
...
def run_experiment(
    ...
    if not all(coverage.values()):
        ...
        raise ValueError(f"ABYAX coverage gate failed: {', '.join(failed)}")
    quality = abyax_actual_path.nav_quality(prices)
    results = {}
    for leverage in abyax_actual_path.FROZEN_LEVERAGES:
        returns, detail = abyax_actual_path.run_scenario(
            prices, leverage, entry_sales_load=entry_sales_load,
            exit_sales_load=exit_sales_load,
        )
        validation = abyax_actual_path.evaluate_actual_path(
    ...
    monte_carlo = abyax_actual_path.circular_block_monte_carlo_chunked(
        primary, simulations=simulations, block_length=63, seed=20140701
    )
    period = pd.Series([
    ...
    event = pd.Series([
        str(year) if year in (2018, 2020, 2022, 2025) else "other_days"
        for year in primary.index.year
    ], index=primary.index)
    regimes = pd.concat([
...
def main():
    ...
    output = root / "reports/mini_medallion_abyax_actual_path"
    inputs = output / "inputs"
    output.mkdir(parents=True, exist_ok=True)
    prices = load_adjusted_close(
        "ABYAX", START, END, cache_folder=inputs, refresh=False
    )
