"""Monthly TQQQ/SQQQ switch driven by the lagged QQQ 200-day trend."""
import pandas as pd

from cboe_options_benchmark import summarize_returns


def build_switch_positions(qqq, tqqq, sqqq):
    levels = pd.concat(
        [qqq.rename("QQQ"), tqqq.rename("TQQQ"), sqqq.rename("SQQQ")],
        axis=1, join="inner"
    ).dropna()
    ...
    targets = pd.DataFrame({
        "TQQQ": target_tqqq,
        "SQQQ": (1.0 - target_tqqq).where(valid),
    }, index=levels.index)
...
def run_switch_sleeve(
    qqq, tqqq, sqqq, start="2016-09-12", end="2026-09-10",
    ...
    positions = build_switch_positions(qqq, tqqq, sqqq)
    returns = levels[["TQQQ", "SQQQ"]].pct_change(fill_method=None).fillna(0.0)
    ...
    simulation["sqqq_position"] = positions["SQQQ"].values
...
def align_with_btc(sleeve_simulation, btc_simulation):
    sleeve = sleeve_simulation.copy()
    sleeve["open_time"] = pd.to_datetime(sleeve["open_time"], utc=True)
    sleeve_equity = sleeve.set_index("open_time")["equity"].sort_index()
    ...
    common_dates = sleeve_equity.index.intersection(btc_equity.index)
    return pd.DataFrame({
        "TQQQ_SQQQ_SWITCH": sleeve_equity.reindex(common_dates)
        .pct_change(fill_method=None).fillna(0.0),
        "BTC_TSMOM30_DD": btc_equity.reindex(common_dates)
        .pct_change(fill_method=None).fillna(0.0),
    }, index=common_dates)
