"""Run the preregistered SOXX/SOXL trend long-short first-read OOS study."""
...
def evaluate_long_oos(returns):
    validation = strict_validation.evaluate_strict_oos(
        returns, independent_oos=True, costs_included=True, minimum_years=16.0
    )
...
def run_experiment(prices, simulations=5000):
    ...
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 16.0:
        raise ValueError("Semiconductor common panel must span at least sixteen years")
    inception_covered = prices.index[0] <= strategy.LATEST_ACCEPTABLE_START
    results = {}
    for multiplier in (1.0, 2.0, 3.0):
        returns, detail, weights, targets = strategy.run_scenario(prices, multiplier)
        ...
        results[multiplier] = {
            ...
            "targets": targets, "validation": validation,
        }
    primary = results[1.0]["returns"]
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=20100311
    )
    periods = pd.Series([
        "2010_2015" if year <= 2015 else
        "2016_2020" if year <= 2020 else "2021_plus"
        for year in primary.index.year
    ], index=primary.index)
    events = pd.Series([
        str(year) if year in (2011, 2018, 2020, 2022) else "other_days"
        for year in primary.index.year
    ], index=primary.index)
    regimes = pd.concat([
...
def main():
    ...
    output = root / "reports/mini_medallion_semiconductor_trend_oos"
    ...
    diagnostics = pd.DataFrame([{
        "long_soxl_days": int(primary["weight_SOXL"].gt(0).sum()),
        "short_soxx_days": int(primary["weight_SOXX"].lt(0).sum()),
    ...
    diagnostics.to_csv(output / "position_diagnostics_1x.csv", index=False)
    ...
    print(summary[["multiplier", "oos__cagr", "oos__sharpe_ratio", "oos__max_drawdown",
    ...
    print("POSITION_DIAGNOSTICS_1x", diagnostics.iloc[0].to_dict())
