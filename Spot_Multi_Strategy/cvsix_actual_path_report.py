"""Run the preregistered CVSIX long-lived market-neutral-income path study."""
...
def coverage_checks(prices):
    prices = cvsix_actual_path.validate_prices(prices)
    ...
    return {
        "start_by_1991_12_31": prices.index[0] <= LATEST_START,
        "end_by_2026_08_31": prices.index[-1] >= EARLIEST_END,
        "span_at_least_34_5y": (
            (prices.index[-1] - prices.index[0]).days / 365.25 >= 34.5
        ),
...
def run_experiment(
    ...
    if not all(coverage.values()):
        ...
        raise ValueError(f"CVSIX coverage gate failed: {', '.join(failed)}")
    results = {}
    for leverage in cvsix_actual_path.FROZEN_LEVERAGES:
        returns, detail = cvsix_actual_path.run_scenario(
            prices, leverage, entry_sales_load=entry_sales_load,
            exit_sales_load=exit_sales_load,
        )
        validation = cvsix_actual_path.evaluate_actual_path(
    ...
    monte_carlo = cvsix_actual_path.circular_block_monte_carlo_chunked(
        primary, simulations=simulations, block_length=63, seed=19900904
    )
    period = pd.Series([
        "inception_2002" if year <= 2002 else
        "2003_2012" if year <= 2012 else
        "2013_2021" if year <= 2021 else "2022_plus"
        for year in primary.index.year
    ], index=primary.index)
    event = pd.Series([
        str(year) if year in (1998, 2008, 2020, 2022) else "other_days"
        for year in primary.index.year
    ], index=primary.index)
    regimes = pd.concat([
...
def main():
    ...
    output = root / "reports/mini_medallion_cvsix_actual_path"
    inputs = output / "inputs"
    output.mkdir(parents=True, exist_ok=True)
    prices = load_adjusted_close(
        "CVSIX", START, END, cache_folder=inputs, refresh=False
    )
