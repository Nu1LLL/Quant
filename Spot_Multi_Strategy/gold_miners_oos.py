"""独立黄金矿业股相对黄金本身OOS：做多GDX（黄金矿业股，
VanEck）、做空GLD（黄金本身），按月再平衡至美元中性，隔离矿业股
相对金价本身的额外风险溢价。规则、成本、样本区间见
reports/mini_medallion_gold_miners_oos/PREREGISTRATION.md，本
模块结构与其他配对实验一致（同样是美元中性、月度再平衡的两腿
组合），单独成文件以保持本项目"一个假说一个文件"的既有约定。
"""
import numpy as np
import pandas as pd


def validate_pair(miners, bullion, maximum_gap_days=10):
    """对齐GDX/GLD调整收盘价，检查重复日期、非正值和异常缺口。"""
    miners = pd.Series(miners).dropna().astype(float).sort_index()
    bullion = pd.Series(bullion).dropna().astype(float).sort_index()
    if miners.index.has_duplicates or bullion.index.has_duplicates:
        raise ValueError("Duplicate gold-miners price date")
    if (miners <= 0).any() or (bullion <= 0).any():
        raise ValueError("Gold-miners adjusted prices must be positive")
    aligned = pd.concat(
        [miners.rename("GDX"), bullion.rename("GLD")], axis=1, join="inner"
    ).dropna()
    if aligned.empty:
        raise ValueError("GDX and GLD have no overlapping dates")
    gaps = aligned.index.to_series().diff().dt.days.dropna()
    if len(gaps) and gaps.max() > maximum_gap_days:
        raise ValueError(
            f"Abnormally long GDX/GLD price gap: {int(gaps.max())} days"
        )
    return aligned
...
def run_scenario(
    ...
    """月度再平衡至(leverage*0.5多GDX, leverage*0.5空GLD)，日间权重
    随价格自然漂移，漂移用组合净收益（已扣成本）做归一化。
    """
    aligned = validate_pair(miners, bullion)
    ...
    for i, (_, row) in enumerate(returns.iterrows()):
        ...
        day_return = start_miners * row["GDX"] + start_bullion * row["GLD"]
        ...
        weight_miners[i] = start_miners * (1.0 + row["GDX"]) / portfolio_growth
