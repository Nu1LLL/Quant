"""Run the preregistered EMLC EM local-currency-bond carry first-read
OOS study. Reuses qai_oos.validate_prices/run_scenario (same
single-fund leverage/financing model as QAI/DBV/SRRIX/MERFX/DBMF) and
dbv_oos.evaluate_fifteen_year_oos (same 15-year enhanced gate as DBV,
since EMLC's history is long enough to support it).
"""
...
def run_experiment(prices, simulations=5000):
    ...
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 15.0:
        raise ValueError("EMLC history must span at least fifteen years")
    results = {}
    ...
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=20100723
    )
    ...
    event_label = pd.Series([
        str(year) if year in (2013, 2018, 2022) else "other_days"
        for year in primary.index.year
    ], index=primary.index)
    regimes = pd.concat([
...
def main():
    ...
    output = root / "reports/mini_medallion_emlc_oos"
    inputs = output / "inputs"
    prices = load_adjusted_close(
        "EMLC", "2010-07-01", "2026-09-12", cache_folder=inputs, refresh=False
    )
