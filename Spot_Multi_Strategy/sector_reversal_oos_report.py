"""Run the preregistered US sector ETF weekly-reversal OOS study."""
...
def evaluate_long_oos(returns):
    validation = strict_validation.evaluate_strict_oos(
        returns, independent_oos=True, costs_included=True, minimum_years=27.0
    )
    folds = validation["walk_forward_folds"]
    validation["checks"]["walk_forward_passed"] = bool(
        len(folds) >= 25 and not folds.empty and folds["fold_pass"].mean() >= 0.80
    )
...
def run_experiment(prices, simulations=5000):
    ...
    if (prices.index[-1] - prices.index[0]).days / 365.25 < 27.0:
        raise ValueError("Sector common panel must span at least twenty-seven years")
    inception_covered = prices.index[0] <= strategy.LATEST_ACCEPTABLE_START
    results = {}
    for leverage in (1.0, 2.0, 3.0):
        returns, detail, weights, trailing, targets = strategy.run_scenario(prices, leverage)
        ...
        results[leverage] = {
            ...
            "trailing": trailing, "targets": targets, "validation": validation,
        }
    primary = results[1.0]["returns"]
    monte_carlo = strict_validation.circular_block_monte_carlo(
        primary, simulations=simulations, block_length=21, seed=19981222
    )
    periods = pd.Series([
        "1998_2006" if year <= 2006 else
        "2007_2015" if year <= 2015 else "2016_2026"
        for year in primary.index.year
    ], index=primary.index)
    events = pd.Series([
        str(year) if year in (2000, 2008, 2020, 2022) else "other_days"
        for year in primary.index.year
    ], index=primary.index)
    regimes = pd.concat([
...
def main():
    ...
    output = root / "reports/mini_medallion_sector_reversal_oos"
