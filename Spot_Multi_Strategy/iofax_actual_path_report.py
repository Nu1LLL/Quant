"""Run the preregistered IOFAX mortgage-credit path study."""
...
def coverage_checks(prices):
    prices = iofax_actual_path.validate_prices(prices)
    ...
    return {
        "start_by_2016_06_30": prices.index[0] <= LATEST_START,
        "end_by_2026_08_31": prices.index[-1] >= EARLIEST_END,
        "span_at_least_10y": (
            (prices.index[-1] - prices.index[0]).days / 365.25 >= 10.0
        ),
...
def run_experiment(
    ...
    if not all(coverage.values()):
        ...
        raise ValueError(f"IOFAX coverage gate failed: {', '.join(failed)}")
    quality = iofax_actual_path.nav_quality(prices)
    results = {}
    for leverage in iofax_actual_path.FROZEN_LEVERAGES:
        returns, detail = iofax_actual_path.run_scenario(
            prices, leverage, entry_sales_load=entry_sales_load,
            exit_sales_load=exit_sales_load,
        )
        validation = iofax_actual_path.evaluate_actual_path(
    ...
    monte_carlo = iofax_actual_path.circular_block_monte_carlo_chunked(
        primary, simulations=simulations, block_length=63, seed=20150528
    )
    period = pd.Series([
...
def main():
    ...
    output = root / "reports/mini_medallion_iofax_actual_path"
    inputs = output / "inputs"
    output.mkdir(parents=True, exist_ok=True)
    prices = load_adjusted_close(
        "IOFAX", START, END, cache_folder=inputs, refresh=False
    )
