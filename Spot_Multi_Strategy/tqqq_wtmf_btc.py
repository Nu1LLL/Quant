"""Monthly TQQQ/WTMF regime switch driven by the QQQ 200-day trend."""
import pandas as pd

from cboe_options_benchmark import summarize_returns


def build_switch_positions(qqq, tqqq, wtmf):
    levels = pd.concat(
        [qqq.rename("QQQ"), tqqq.rename("TQQQ"), wtmf.rename("WTMF")],
        axis=1, join="inner"
    ).dropna()
    ...
    targets = pd.DataFrame({
        "TQQQ": target_tqqq,
        "WTMF": (1.0 - target_tqqq).where(valid),
    }, index=levels.index)
...
def run_switch_sleeve(
    qqq, tqqq, wtmf, start="2016-09-12", end="2026-09-10",
    ...
    positions = build_switch_positions(qqq, tqqq, wtmf)
    returns = levels[["TQQQ", "WTMF"]].pct_change(fill_method=None).fillna(0.0)
    ...
    simulation["wtmf_position"] = positions["WTMF"].values
