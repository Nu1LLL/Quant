"""Dollar-neutral SCHD versus VTI dividend-quality factor model."""
import numpy as np
import pandas as pd


SYMBOLS = ("SCHD", "VTI")
LATEST_ACCEPTABLE_START = pd.Timestamp("2011-11-30", tz="UTC")
...
def validate_prices(prices):
    ...
    if frame.empty or frame.index.has_duplicates or not np.isfinite(values).all():
        raise ValueError("Dividend-quality panel is empty, duplicated or non-finite")
    if (values <= 0).any():
        raise ValueError("Dividend-quality prices must be positive")
    gaps = frame.index.to_series().diff().dropna().dt.days
    if not gaps.empty and int(gaps.max()) > 10:
        raise ValueError("Dividend-quality common panel has a gap longer than ten days")
    return frame
...
def run_scenario(
    ...
    target = pd.DataFrame({"SCHD": 0.5, "VTI": -0.5}, index=prices.index)
    ...
    short_borrow = weights["VTI"].clip(upper=0.0).abs() * annual_borrow / 252.0
    ...
    detail = pd.DataFrame({
        "weight_SCHD": weights["SCHD"], "weight_VTI": weights["VTI"],
