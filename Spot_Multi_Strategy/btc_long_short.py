"""Fixed 30-observation BTC long/short time-series momentum."""
import numpy as np
import pandas as pd
...
def build_positions(price):
    momentum = np.log(price / price.shift(30))
    target = pd.Series(
        np.where(momentum > 0.0, 1.0, -1.0), index=price.index
    ).where(momentum.notna(), 0.0)
    return target.shift(1).fillna(0.0)


def run_backtest(
    price, leverage=1.0, start="2016-09-12", end="2026-09-10",
    transaction_cost=0.00125, annual_short_cost=0.08,
    annual_financing=0.04, initial_capital=10000.0
):
    position = build_positions(price) * leverage
    asset_return = price.pct_change(fill_method=None).fillna(0.0)
    frame = pd.DataFrame({
        "price": price, "asset_return": asset_return, "position": position
    ...
    net_pnl = (
        ...
        - trading_cost - short_cost - financing_cost
    )
    simulation = pd.DataFrame({
        ...
        "short_cost": short_cost.values,
        "financing_cost": financing_cost.values,
        "cost": (trading_cost + short_cost + financing_cost).values,
        "net_pnl": net_pnl.values,
        "equity": (1.0 + net_pnl).cumprod().values,
        "rebalanced": (turnover > 0.0).values,
    })
    ...
    metrics.update({
        ...
        "total_short_cost": float(short_cost.sum() * initial_capital),
